	...
	*/
	...
	public function resizeImage($file, $width, $height, $avoid_cropping = null, $unique = null, $cache = true, $avoid_resize_small = 0)
	{
		if (is_file($file))
		{
			...
			$width = (int)$width;
			$height = (int)$height;
			$avoid_resize_small = $avoid_resize_small ?: '';
			$file_info = pathinfo($file);
			if ($cache) {
				$thumbfile = $this->app->path->path('media:zoo') . '/images/' . $file_info['filename'] . '_' . md5($file . $width . $height . $avoid_cropping . $avoid_resize_small . $unique) . '.' . $file_info['extension'];
			} else {
				$thumbfile = $file;
			}
			...
			// Prevent of resizing small images by overriding final dimensions
			if ($avoid_resize_small) {
				if ($file_size[0] < $width) {
					$width = $file_size[0];
				}
				if ($file_size[1] < $height) {
					$height = $file_size[1];
				}
			}
