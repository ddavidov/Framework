...
class ZluxController extends AppController {
	...
	*/
	...
	public function upload()
	{
		...
		// resize if width or height is set
		if (isset($_REQUEST['height']) || isset($_REQUEST['width'])) {
			$filePath_abs = $this->app->path->path('root:' . $filePath_rel);
			$resized_image = $this->app->zlfw->resizeImage($filePath_abs, (int)$_REQUEST['width'], (int)$_REQUEST['height'], (int)($_REQUEST['crop'] == false), $unique = null, $cache = false, (isset($_REQUEST['avoid_resize_small']) ? (int)$_REQUEST['avoid_resize_small'] : 0));
			$filePath_rel = $this->app->path->relative($resized_image);
		}
