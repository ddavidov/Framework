...
class zlfwHelperString extends AppHelper {
	...
	{	
		...
		if (isset($length) && $length > 0 && $mode == 1)
		{
			...
			for ($i = 0; $i < count($values); $i++) {
				$text = $values[$i];
				$result_len += strlen($text);
				if ($result_len > $length){
					$values[$i] = $this->truncate($text, $length, $truncate_string);
					$values = array_slice($values, 0, $i+1); // remove the rest
					$report['limited'] = true;
					break;
				} else { $values[$i] = $text; }
			}
		...
		else if (isset($length) && $length > 0 && $mode == 2)
		{
			$limited_once = false;
			for ($i = 0; $i < count($values); $i++) {
				if (strlen($values[$i]) > $length){
					$values[$i] = $this->truncate($values[$i], $length, $truncate_string);
					$limited_once == true;
					$report['limited'] = $limited_once ? $report['limited'] : true;
				}
			}
		}
	...
	*/
	public function truncate($text, $length = 30, $truncate_string = '...', $fixHtml = false) {
