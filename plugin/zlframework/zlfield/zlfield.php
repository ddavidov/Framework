...
class ZlfieldHelper extends AppHelper {
	...
	public function __construct($default = array())
	{
		...
		// get option to compare with elements count
		if ($this->app->zlfw->environment->is('admin.com_zoo.manager')) {
			$max_input_vars = ini_get('max_input_vars');
			$this->app->document->addScriptDeclaration("var zl_options = {max_input_vars:$max_input_vars};");
		}
	}
	...
	{
		$this->config = array();
		if(($this->controller == 'manager' || $this->controller == 'zlframework') && !empty($this->type) && $type = $this->application->getType($this->type))
		{
	...
	*/
	...
	public function loadAssets()
	{
		if (!$this->loadedAssets) {
			...
			// load wk2 assets
			if (JFile::exists(JPATH_ADMINISTRATOR . '/components/com_widgetkit/widgetkit.xml')) {
