	...
	*/
	...
	public function wk2Field($id, $name, $value, $spec, $attrs, $returnRawValue)
	{
		...
		if (!$app = @include(JPATH_ADMINISTRATOR . '/components/com_widgetkit/widgetkit-app.php')) {
            return '<input type="hidden" name="'.$name.'" value="'.$value.'">' . JText::_('PLG_ZLFRAMEWORK_WK2_NOT_PRESENT');
        }

        $app->trigger('init.admin', array($app));

        if (!isset($app['plugins']['content/zoopro']) || !isset($app['plugins']['content/zoo'])) {
        	return '<input type="hidden" name="'.$name.'" value="'.$value.'">' . JText::_('PLG_ZLFRAMEWORK_WK2_ZLPLUGIN_MISSING');
        }
